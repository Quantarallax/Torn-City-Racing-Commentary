...
(function () {
    ...
    const ICON = {
        ...
        // Per spec v3.6: medium-grey clock icon for countdown-timer
        // milestone messages (45/30/15/5/1/30s).
    ...
    const LINES = {
        COUNTDOWN: {
            ...
            // Per spec v3.6: radio-comms sub-pool. Pickups from a paddock
            // scanner of racer-to-crew and crew-to-racer messages -
            // distinct from the police scanner. No specific words are
            // quoted - instead the FEELING/TONE of the exchange is
    ...
    function ambientPoolFor (statusLines) {
        ...
        if (statusLines === LINES.PRE_LAUNCH || statusLines === LINES.COUNTDOWN) {
            // Per spec v3.6: paddock-radio-comms sub-pool merges into
            // COUNTDOWN ambient (not PRE-LAUNCH which has its own entry
            // pool). Lines describe the tone of cockpit-to-crew exchanges.
            ...
            if (secsLeft !== null && secsLeft > 0) {
                ...
                for (let i = 0; i < COUNTDOWN_MILESTONES.length; i++) {
                    ...
                    if (secsLeft <= ms.sec && !firedMilestones[ms.sec]) {
                        ...
                        // (status type = alwaysShow, not throttled by
                        // ambient gap) with a grey clock icon. Text is
                        // wrapped in .fl-milestone for medium-grey
                        // colour. isHtml=true so the wrapping span is
                        // honoured rather than escaped.
                        const filled = fill(tpl).replace(/\{timeLeft\}/g, ms.label);
                        pushLine(
                            '<span class="fl-milestone">' + escH(filled) + '</span>',
                            'status', ICON.milestone, true
                        );
                        break;
                    }
                }
    ...
    // Per spec v3.6: commentary for non-player finishers, fired only
    // while the player is still racing. About half of all finisher
    // lines also pull a contextual nearby-racer reference - either
    // the gap to the previous finisher or the still-active racer
    // immediately behind in race order.
    const OTHER_FINISH_PLAIN = [
        '{name} crosses the line. A solid run on {track}.',
    ...
    const OTHER_FINISH_WITH_PREV = [
        ...
        '{name} takes the flag {gap} after {prev}. They couldn\'t quite close it.',
        '{name} comes home {gap} adrift of {prev}. So close all race.',
        'Tight one - {name} only {gap} off {prev} at the line.',
        '{name} finishes {gap} behind {prev}. They were never far apart.'
    ];
    const OTHER_FINISH_WITH_BEHIND = [
        '{name} crosses the line, with {behind} still hard at it just behind.',
        '{name} takes the chequer - {behind} now the next car still going.',
        '{name} is in, leaving {behind} to scrap for what is left.',
        '{name} done and dusted, with {behind} still grinding it out behind.',
        '{name} finishes ahead of {behind}, who is still racing for position.'
    ];

    function fireOtherFinisherLine (entry) {
        const name = entry.name;
        const r = Math.random();
        let tpl = null;
        // PREV-GAP branch: need a prior finisher AND a parseable gap.
        if (r < 0.25 && state.finishers.length >= 2) {
            const prev = state.finishers[state.finishers.length - 2];
            const gap = computeFinishGap(prev.time, entry.time);
            if (prev && prev.name && gap) {
                tpl = pickLine(OTHER_FINISH_WITH_PREV, 'finish')
                    .replace(/\{prev\}/g, prev.name)
                    .replace(/\{gap\}/g, gap);
            }
        }
        // BEHIND branch: need a still-active racer (not the player, not
        // this finisher).
        if (!tpl && r < 0.5) {
            const active = getActiveRacers();
            let behindName = null;
            for (let i = 0; i < active.length; i++) {
                const n = active[i] && active[i].name;
                if (n && n !== name && n !== state.playerName) {
                    behindName = n;
                    break;
                }
            }
            if (behindName) {
                tpl = pickLine(OTHER_FINISH_WITH_BEHIND, 'finish')
                    .replace(/\{behind\}/g, behindName);
            }
        }
        // Plain fallback.
        if (!tpl) tpl = pickLine(OTHER_FINISH_PLAIN, 'finish');
        const filled = tpl.replace(/\{name\}/g, name)
                          .replace(/\{track\}/g, state.track || 'the track');
        pushLine(filled, 'finish', ICON.flag);
    }

    // Parse two MM:SS.ss (or HH:MM:SS.ss) finish times and return a
    // human-readable gap string like "2.3s" or "1:04". Returns '' if
    // either string can't be parsed.
    function computeFinishGap (prevStr, nextStr) {
        function toSec (str) {
            if (!str) return null;
        ...
        const a = toSec(prevStr);
        const b = toSec(nextStr);
